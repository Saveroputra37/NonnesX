...
const Usercreatepage = () => {
  const { signUp: _signUp, setSession, isLoaded } = useSignUp();
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        try {
          ...
          setError("Gagal mengirim kode verifikasi. " + (prepareErr.errors?.[0]?.message || ""));
          return;
        }
      ...
      const errorMessage = err.errors?.[0]?.message || "Terjadi kesalahan saat membuat akun";
  ...
  const handleVerifyEmail = async (e) => {
    ...
    try {
      ...
      const errorMessage = err.errors?.[0]?.message || "Kode verifikasi tidak valid";
      console.error("Verification Error:", err);
      console.error("Error details:", err.errors);
      
      // Better error messages
      if (errorMessage.includes("Incorrect code")) {
        setError("Kode verifikasi salah. Periksa kembali kode di email Anda atau minta kode baru.");
      } else if (errorMessage.includes("send a verification code")) {
        setError("Silakan klik 'Kirim Ulang Kode' untuk menerima kode verifikasi baru");
  ...
  const handleResendVerification = async () => {
    ...
    setIsResending(true);
    try {
      console.log("Mengirim ulang verification email untuk signup ID:", signUpId);
      // Resend verification code
      const result = await _signUp.prepareEmailAddressVerification();
      
      console.log("Resend result:", result);
      setError(`Kode verifikasi baru telah dikirim ke email: ${email}. Periksa inbox atau folder spam.`);
    } catch (err) {
      const errorMessage = err.errors?.[0]?.message || "Gagal mengirim ulang kode verifikasi";
      setError(errorMessage);
        ...
        <div className="bg-white p-8 rounded-lg lg:w-full h-full sm:w-full md:w-full shadow-lg relative">
          ...
          <div className="mt-10">
            ...
            <p className="text-center mt-2 text-gray-600/70 capitalize xl:text-lg md:text-md xl:w-[90%] mx-auto">
              Buat Akun Anda sekarang dan jadilah bagian dari keluarga di era aplikasi berita sosial!
            </p>
          </div>
          <form
          ...
          >
            {!isVerifying ? (
              ...
              <div className="my-5">
                ...
                >
                  {verificationCode.trim().length === 6 ? "Verifikasi Sekarang" : "Tunggu 6 digit kode"}
                </button>
                <button
                  type="button"
                  onClick={() => {
            ...
            <p className="text-center text-sm text-gray-600/70 absolute bottom-5 left-7 w-10/12 border-t-2 border-gray-200 pt-3">
              Dengan mendaftar, Anda setuju dengan Ketentuan dan Kebijakan Privasi kami.
            </p>
          </form>
        </div>
  );
